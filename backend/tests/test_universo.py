...
def test_conservador_no_tiene_materias_primas_volatiles():
    ...
    assert "SSLN.L" not in universo
    assert "ICOM.L" not in universo
    assert "SGLN.L" in universo
...
def test_agresivo_ve_todo():
    ...
    assert "SSLN.L" in universo
    assert "EXXT.DE" in universo
    assert "XMME.DE" in universo
...
def test_conservador_no_tiene_sectoriales():
    ...
    """
    universo = filtrar_por_perfil(NivelRiesgo.CONSERVADOR)
    for sectorial in ["XUTC.DE", "XUEN.DE", "XUFN.DE"]:
...
def test_diversificacion_regional_detecta_concentracion():
    ...
    """
    activos = ["VUSA.AS", "EXXT.DE", "VHVE.L", "IMAE.AS", "SUAG.L", "SGLN.L"]
    ...
    pesos = [0.35, 0.30, 0.15, 0.10, 0.10, 0.00]        # VUSA.AS+EXXT.DE+SUAG.L = 75% EEUU
